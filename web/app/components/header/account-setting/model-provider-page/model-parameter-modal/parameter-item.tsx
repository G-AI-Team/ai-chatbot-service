...
const ParameterItem: FC<ParameterItemProps> = ({
  ...
  const renderInput = () => {
    ...
    if (parameterRule.type === 'int') {
      ...
      return (
        <>
          ...
          <input
            ref={numberInputRef}
            className="system-sm-regular ml-4 block h-8 w-16 shrink-0 appearance-none rounded-md bg-components-input-bg-normal pl-3 text-components-input-text-filled outline-none"
            type="number"
            max={parameterRule.max}
            min={parameterRule.min}
    ...
    if (parameterRule.type === 'string' && !parameterRule.options?.length) {
      return (
        <input
          className={cn(isInWorkflow ? 'w-[150px]' : 'w-full', 'system-sm-regular ml-4 flex h-8 appearance-none items-center rounded-md bg-components-input-bg-normal px-3 text-components-input-text-filled outline-none')}
    ...
    if (parameterRule.type === 'text') {
      return (
        <textarea
          className="system-sm-regular ml-4 h-20 w-full rounded-md bg-components-input-bg-normal px-1 text-components-input-text-filled"
          value={renderValue as string}
          onChange={handleStringInputChange}
        />
      )
