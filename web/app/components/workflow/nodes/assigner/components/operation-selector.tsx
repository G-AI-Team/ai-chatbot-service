...
const OperationSelector: FC<OperationSelectorProps> = ({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={cn('flex items-center gap-0.5 rounded-md bg-components-input-bg-normal px-2 py-1', disabled ? 'cursor-not-allowed !bg-components-input-bg-disabled' : 'cursor-pointer hover:bg-state-base-hover-alt', open && 'bg-state-base-hover-alt', className)}
        >
      ...
      <PortalToFollowElemContent className={`z-20 ${popupClassName}`}>
        <div className="flex w-[140px] flex-col items-start rounded-md border-[0.5px] border-components-panel-border bg-components-panel-bg-blur shadow-lg">
            ...
            {items.map(item => (
              !isOperationItem(item)
                ...
                : (
                  <div
                    key={item.value}
                    className={cn('flex items-center gap-1 self-stretch rounded-md px-2 py-1', 'cursor-pointer hover:bg-state-base-hover')}
                    onClick={() => {
                      onSelect(item)
                      setOpen(false)
                    }}
