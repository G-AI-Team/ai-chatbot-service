...
const SuggestedAction = ({ icon, link, disabled, children, className, onClick, ...props }: SuggestedActionProps) => {
  ...
  return (
    <a
      ...
      target="_blank"
      rel="noreferrer"
      className={cn('flex items-center justify-start gap-2 rounded-md bg-background-section-burn px-2.5 py-2 text-text-secondary transition-colors [&:not(:first-child)]:mt-1', disabled ? 'cursor-not-allowed opacity-30 shadow-xs' : 'cursor-pointer text-text-secondary hover:bg-state-accent-hover hover:text-text-accent', className)}
      onClick={handleClick}
      {...props}
    >
